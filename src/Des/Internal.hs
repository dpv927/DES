...
permFromList64 (x:xs) destBit block initBlock = permFromList64 xs (destBit+1) newBlock initBlock
  where 
    newBlock = if Des.Bits.testBit64 initBlock x then 
      Des.Bits.setBit64 block destBit
    else block
...
permFromList32 (x:xs) destBit block initBlock = permFromList32 xs (destBit+1) newBlock initBlock
  where 
    newBlock = if Des.Bits.testBit32 initBlock x then 
      Des.Bits.setBit32 block destBit
    else block
...
ks c d iter = (permPC2 $ Des.Bits.joinCD cn dn, cn, dn)
  where 
    shifts = cdShifts ! iter
    cn = Des.Bits.rotateBlock28Left c shifts
    dn = Des.Bits.rotateBlock28Left d shifts
